...
const validationSchema = Joi.object({
  ...
  genre_id: Joi.number().required().messages({
    'integer.empty':'Please enter the genre id of the game',
  }),
...
const EditItem = ({game_id, game_title, game_image_big, game_image_small, game_logo, release_date, description ,about, game_url, genre_id, developer, publisher}) => {
  ...
  const onSubmitForm = async (data) =>{
    try {
      ...
      const response = await fetch(`http://localhost:5000/games/edit/${game_id}`,
      {
        method: "PUT",
      ...
      goHome();
      toast.success("Edited game succesfully!");    
  ...
  return(
    <div className='dash-bg'>
      <div className='reg-container'>
        ...
        <div className='signup'>Edit game</div>
        <form className='form' onSubmit={handleSubmit(onSubmit)}>
          <Controller
            control={control}
            name="game_title"
            defaultValue={game_title}
            render={({field}) => (
              <TextField 
                {...field} 
                autoComplete='off' 
